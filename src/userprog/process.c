...
process_execute (const char *file_name) 
{
  ...
  name_of_file = malloc(strlen(file_name)+1);
  strlcpy (name_of_file, file_name, strlen(file_name)+1);

  char *save_ptr;
  name_of_file = strtok_r (name_of_file," ",&save_ptr);
  ...
  sema_down(&thread_current()->process_child_lock);
...
process_wait (tid_t child_tid) 
{
  ...
  thread_current()->wait_on_tid = ch->tid;
